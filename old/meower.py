...
class Meower:
    def __init__(self, cl, extra_data):
        self.cl = cl
        # Changes to imports bc of CL4.
        self.supporter = extra_data["supporter"]
        self.log = extra_data["logger"]
        self.errorhandler = extra_data["errorhandler"]
        self.accounts = extra_data["accounts"]
        self.filesystem = extra_data["files"]
        self.sendPacket = self.supporter.sendPacket
        
        # Need to specify stuff to ignore in this server.
    ...
    def ping(self, client, val, listener_detected, listener_id, room_id):
    ...
    def version_chk(self, client, val, listener_detected, listener_id, room_id):
        if type(val) == str:
    ...
    def get_ulist(self, client, val, listener_detected, listener_id, room_id):
        self.sendPacket({"cmd": "ulist", "val": self.cl._get_ulist(), "id": client})

    # Accounts and security
    
    def authpswd(self, client, val, listener_detected, listener_id, room_id):
    ...
    def gen_account(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if not self.supporter.isAuthenticated(client):
            if type(val) == dict:
                if ("username" in val) and ("pswd" in val):
    ...
    def get_profile(self, client, val, listener_detected, listener_id, room_id):
    ...
    def update_config(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == dict:
    ...
    def get_home(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
    ...
    def post_home(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
                if not len(val) > 360:
    ...
    def get_post(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
    ...
    def get_peak_users(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            payload = {
                "mode": "peak",
    ...
    def search_user_posts(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == dict:
    ...
    def report(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == dict:
    ...
    def close_report(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def clear_home(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def clear_user_posts(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
                FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
    ...
    def alert(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def announce(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def block(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def unblock(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def kick(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def get_user_ip(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def get_ip_data(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def get_user_data(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def ban(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def pardon(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def terminate(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
                FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
    ...
    def repair_mode(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def delete_post(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
    ...
    def create_chat(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
                if not len(val) > 20:
    ...
    def leave_chat(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
                if not len(val) > 50:
    ...
    def get_chat_list(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if self.supporter.isAuthenticated(client):
    ...
    def get_chat_data(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
                if not len(val) > 50:
    ...
    def get_chat_posts(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
                if not len(val) > 50:
    ...
    def set_chat_state(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if not self.supporter.isAuthenticated(client):
            # Not authenticated
    ...
    def post_chat(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
    ...
    def add_to_chat(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == dict:
    ...
    def remove_from_chat(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is already authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == dict:
                if (("username" in val) and (type(val["username"]) == str)) and (("chatid" in val) and (type(val["chatid"]) == str)):
    ...
    def get_inbox(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == dict:
                if ("page" in val) and self.checkForInt(val["page"]):
    ...
    def change_pswd(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            if type(val) == str:
    ...
    def del_tokens(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
    ...
    def del_account(self, client, val, listener_detected, listener_id, room_id):
        # Check if the client is authenticated
        if self.supporter.isAuthenticated(client):
            FileCheck, FileRead, accountData = self.accounts.get_account(client, True, True)
            if FileCheck and FileRead:
