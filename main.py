...
class Main:
    def __init__(self, debug=False):
        ...
        self.supporter = Supporter(  # Support functionality
            cl=self.cl, packet_callback=self.handle_packet
        )
        self.filesystem = Files(  # Filesystem/Database I/O
            logger=self.supporter.log, errorhandler=self.supporter.full_stack
        )
        ...
        self.cl.server(
            port=int(os.getenv("CL3_PORT", 3000)), ip=os.getenv("CL3_HOST", "")
        )

    def returnCode(self, client, code, listener_detected, listener_id):
        self.supporter.sendPacket(
    ...
    def handle_packet(
        self, cmd, ip, val, listener_detected, listener_id, client, clienttype
    ):
        try:
            if cmd in COMMANDS:
