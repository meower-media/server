...
def password():
    ...
    if request.method == "GET":
        ...
        meower.db["usersv0"].update_one({"_id": request.user._id}, {"$set": {"security.password": {"hash_type": "scrypt", "hash": hashed_password}}})
        ...
        # Remove password
        meower.db["usersv0"].update_one({"_id": request.user._id}, {"$set": {"security.password": None}})
...
def totp():
    ...
    if request.method == "GET":
        ...
        meower.db["usersv0"].update_one({"_id": request.user._id}, {"$set": {"security.totp": {"secret": secret, "recovery_codes": recovery_codes}}})
        ...
        # Remove TOTP
        meower.db["usersv0"].update_one({"_id": request.user._id}, {"$set": {"security.totp": None}})
...
def get_config():
    if request.method == "GET":
        ...
        # Update config
        meower.db["usersv0"].update_one({"_id": request.user._id}, {"$set": {"username": userdata["username"], "lower_username": userdata["lower_username"], "config": userdata["config"], "profile": userdata["profile"]}})
...
def blocked_users():
    ...
    if request.method == "GET":
        ...
        if blocked_user._id not in userdata["security"]["blocked"]:
            ...
            meower.db["usersv0"].update_one({"_id": request.user._id}, {"$set": {"security.blocked": userdata["security"]["blocked"]}})
