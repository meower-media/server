...
class Meower:
    def __init__(self, cl, supporter, logger, errorhandler, security, files):
        ...
        self.log(
            f"Successfully loaded {len(self.supporter.blocked_ips.nodes())} netblock(s) into Radix!"
        )
        self.log(
            f"Successfully loaded {len(self.supporter.registration_blocked_ips.nodes())} registration netblock(s) into Radix!"
        )
    ...
    def returnCode(self, client, code, listener_detected, listener_id):
        self.sendPacket(
            {"cmd": "statuscode", "val": self.cl.codes[str(code)], "id": client},
    ...
    def get_ulist(self, client, val, listener_detected, listener_id):
        self.sendPacket(
            {"cmd": "ulist", "val": self.cl._get_ulist(), "id": client},
            listener_detected=listener_detected,
            listener_id=listener_id,
        )
    ...
    def authpswd(self, client, val, listener_detected, listener_id):
        ...
        elif (account["flags"] & UserFlags.DELETED == UserFlags.DELETED) or (
            account["delete_after"] and account["delete_after"] <= time.time() + 60
        ):
        ...
        # Check password
        if (password not in account["tokens"]) and (
            not bcrypt.checkpw(password.encode(), account["pswd"].encode())
        ):
            self.supporter.ratelimit(f"login:u:{username}:f", 5, 60)
            return self.returnCode(
                client=client,
        ...
        # Update netlog
        netlog_result = self.files.db.netlog.update_one(
            {"_id": {"ip": ip, "user": username}},
            {"$set": {"last_used": int(time.time())}},
            upsert=True,
        )

        # Check ban
        if (account["ban"]["state"] == "perm_ban") or (
            account["ban"]["state"] == "temp_ban"
            and account["ban"]["expires"] > time.time()
        ):
            self.supporter.ratelimit(f"login:u:{username}:f", 5, 60)
            self.sendPacket(
                {
                    "cmd": "direct",
                    "val": {"mode": "banned", "payload": account["ban"]},
                    "id": client,
                },
                listener_detected=listener_detected,
                listener_id=listener_id,
        ...
        if account["permissions"] and netlog_result.upserted_id:
            self.supporter.createPost(
                "inbox",
                username,
                f"Your account was logged into on a new IP address ({ip})! You are receiving this message because you have admin permissions. Please make sure to keep your account secure.",
            )
        ...
        if account["delete_after"]:
            self.supporter.createPost(
                "inbox",
                username,
                f"Your account was scheduled for deletion but you logged back in. Your account is no longer scheduled for deletion! If you didn't request for your account to be deleted, please change your password immediately.",
            )
        ...
        self.supporter.setAuthenticatedState(
            client, True
        )  # Make the server know that the client is authed
    ...
    def gen_account(self, client, val, listener_detected, listener_id):
        ...
        # Check ratelimit
        if self.supporter.ratelimited(
            f"registration:{ip}:f"
        ) or self.supporter.ratelimited(f"registration:{ip}:s"):
            return self.returnCode(
                client=client,
                code="RateLimit",
                listener_detected=listener_detected,
        ...
        self.files.db.user_settings.insert_one(
            {
                "_id": username,
                "accepted_terms": {LATEST_TERMS_REVISION: int(time.time())},
            }
        )
        ...
        # Update netlog
        self.files.db.netlog.update_one(
            {"_id": {"ip": ip, "user": username}},
            {"$set": {"last_used": int(time.time())}},
            upsert=True,
        )
        ...
        self.supporter.createPost(
            "inbox",
            username,
            "Welcome to Meower! We welcome you with open arms! You can get started by making friends in the global chat or home, or by searching for people and adding them to a group chat. We hope you have fun!",
        )
    ...
    def accept_terms(self, client, val, listener_detected, listener_id):
        ...
        self.files.db.user_settings.update_one(
            {"_id": client},
            {"$set": {"accepted_terms": user_settings["accepted_terms"]}},
        )
        ...
        self.sendPacket(
            {
                ...
                "val": {
                    "mode": "update_config",
                    "payload": {"accepted_terms": user_settings["accepted_terms"]},
                },
                "id": client,
            }
        )
    ...
    def update_config(self, client, val, listener_detected, listener_id):
        ...
        self.sendPacket(
            {
                "cmd": "direct",
                "val": {"mode": "update_config", "payload": val},
                "id": client,
            }
        )
    ...
    def change_pswd(self, client, val, listener_detected, listener_id):
        ...
        self.files.db.usersv0.update_one(
            {"_id": client},
            {
                "$set": {
                    "pswd": bcrypt.hashpw(
                        new_pswd.encode(), bcrypt.gensalt(rounds=14)
                    ).decode()
                }
            },
        )
    ...
    def create_chat(self, client, val, listener_detected, listener_id):
        ...
        if (
            self.files.db.chats.count_documents(
                {"type": 0, "members": client}, limit=150
            )
            >= 150
        ):
            return self.returnCode(
    ...
    def leave_chat(self, client, val, listener_detected, listener_id):
        ...
        chat = self.files.db.chats.find_one(
            {"_id": val, "members": client, "deleted": False}
        )
        if not chat:
            return self.returnCode(
                client=client,
        ...
        if chat["type"] == 0:
            ...
            if len(chat["members"]) > 0:
                ...
                self.files.db.chats.update_one(
                    {"_id": val},
                    {"$set": {"owner": chat["owner"]}, "$pull": {"members": client}},
                )
    ...
    def get_chat_data(self, client, val, listener_detected, listener_id):
        ...
        self.sendPacket(
            {
                "cmd": "direct",
                "val": {"mode": "chat_data", "payload": chat},
                "id": client,
            },
            listener_detected=listener_detected,
            listener_id=listener_id,
    ...
    def add_to_chat(self, client, val, listener_detected, listener_id):
        ...
        # Get chat
        chat = self.files.db.chats.find_one(
            {"_id": chatid, "members": client, "deleted": False}
        )
        if not chat:
            return self.returnCode(
                client=client,
        ...
        user = self.files.db.usersv0.find_one(
        ...
        if (
            self.files.db.relationships.count_documents(
                {
                    "$or": [
                        {"_id": {"from": client, "to": username}, "state": 2},
                        {"_id": {"from": username, "to": client}, "state": 2},
                    ]
                },
                limit=1,
            )
        ...
        self.files.db.chats.update_one(
            {"_id": chatid}, {"$addToSet": {"members": username}}
        )
        ...
        self.sendPacket(
            {
                ...
                "val": {
                    "mode": "update_chat",
                    "payload": {"_id": chatid, "members": chat["members"]},
                },
                "id": chat["members"],
            }
        )
        ...
        self.supporter.createPost(
            "inbox",
            username,
            f"You have been added to the group chat '{chat['nickname']}' by @{client}!",
        )
        ...
        self.supporter.createPost(
            chatid,
            "Server",
            f"@{client} added @{username} to the group chat.",
            chat_members=chat["members"],
        )
    ...
    def remove_from_chat(self, client, val, listener_detected, listener_id):
        ...
        # Get chat
        chat = self.files.db.chats.find_one(
            {"_id": chatid, "members": {"$all": [client, username]}, "deleted": False}
        )
        if not chat:
            return self.returnCode(
                client=client,
        ...
        self.files.db.chats.update_one(
            {"_id": chatid}, {"$pull": {"members": username}}
        )
        ...
        self.sendPacket(
            {"cmd": "direct", "val": {"mode": "delete", "id": chatid}, "id": username}
        )
        ...
        self.supporter.createPost(
            "inbox",
            username,
            f"You have been removed from the group chat '{chat['nickname']}' by @{client}!",
        )
        ...
        self.supporter.createPost(
            chatid,
            "Server",
            f"@{client} removed @{username} from the group chat.",
            chat_members=chat["members"],
        )
    ...
    def set_chat_state(self, client, val, listener_detected, listener_id):
        ...
        if chatid == "livechat":
            self.sendPacket(
                {
                    "cmd": "direct",
                    "val": {"chatid": chatid, "u": client, "state": state},
                }
            )
        else:
            self.sendPacket(
    ...
    def get_home(self, client, val, listener_detected, listener_id):
        ...
        self.sendPacket(
            {
                "cmd": "direct",
                "val": {"mode": "home", "payload": [post["_id"] for post in posts]},
                "id": client,
            },
            listener_detected=listener_detected,
            listener_id=listener_id,
    ...
    def search_user_posts(self, client, val, listener_detected, listener_id):
        ...
        # Get posts
        posts = self.files.db.posts.find(
            {"post_origin": "home", "isDeleted": False, "u": val["query"]},
            projection={"_id": 1},
            sort=[("t.e", pymongo.DESCENDING)],
            skip=(page - 1) * 25,
            limit=25,
        ...
        self.sendPacket(
            {
                "cmd": "direct",
                "val": {"mode": "user_posts", "index": [post["_id"] for post in posts]},
                "id": client,
            },
            listener_detected=listener_detected,
            listener_id=listener_id,
    ...
    def get_inbox(self, client, val, listener_detected, listener_id):
        ...
        self.sendPacket(
            {
                "cmd": "direct",
                "val": {"mode": "inbox", "payload": [post["_id"] for post in posts]},
                "id": client,
            },
            listener_detected=listener_detected,
            listener_id=listener_id,
    ...
    def get_chat_posts(self, client, val, listener_detected, listener_id):
        ...
        if (
            self.files.db.chats.count_documents(
                {"_id": val, "members": client, "deleted": False}, limit=1
            )
            < 1
        ):
            return self.returnCode(
    ...
    def post_chat(self, client, val, listener_detected, listener_id):
        ...
        if chatid != "livechat":
            ...
            if chat["type"] == 1:
                ...
                Thread(
                    target=self.files.db.user_settings.bulk_write,
                    args=(
                        [
                            pymongo.UpdateMany(
                                {
        ...
        # Create post
        FileWrite, _ = self.supporter.createPost(
            chatid,
            client,
            content,
            chat_members=(chat["members"] if chatid != "livechat" else None),
        )
        if not FileWrite:
            return self.returnCode(
                client=client,
    ...
    def get_post(self, client, val, listener_detected, listener_id):
        ...
        # Get post
        post = self.files.db.posts.find_one(
            {"_id": post["post_origin"], "isDeleted": False}
        )
        if not post:
            return self.returnCode(
                client=client,
        ...
        if (post["post_origin"] == "inbox") and (post["u"] != client):
            ...
            if (
                self.files.db.chats.count_documents(
                    {"_id": post["post_origin"], "members": client, "deleted": False},
                    limit=1,
                )
                < 1
            ):
        ...
        # Return post
        self.sendPacket(
            {"cmd": "direct", "val": {"mode": "post", "payload": post}, "id": client},
            listener_detected=listener_detected,
            listener_id=listener_id,
        )
        self.returnCode(
    ...
    def delete_post(
        self, client, val, listener_detected, listener_id
    ):  # TODO: needs checking
        # Check if the client is authenticated
        if not self.supporter.isAuthenticated(client):
            return self.returnCode(
        ...
        # Update post
        self.files.db.posts.update_one(
            {"_id": post["_id"]},
            {"$set": {"isDeleted": True, "deleted_at": int(time.time())}},
        )

        # Send delete post event
        if post["post_origin"] == "home":
            self.sendPacket(
                {"cmd": "direct", "val": {"mode": "delete", "id": post["_id"]}}
            )
        else:
            self.sendPacket(
                {
                    "cmd": "direct",
                    "val": {"mode": "delete", "id": post["_id"]},
                    "id": chat["members"],
                }
            )
    ...
    def report(
        ...
        if (
            (not isinstance(content_type, int))
        ...
        if content_type == 0:
            if (
                self.files.db.posts.count_documents(
                    {"_id": content_id, "post_origin": {"$ne": "inbox"}}, limit=1
                )
                < 1
            ):
                return self.returnCode(
        ...
        self.files.db.reports.update_one(
        ...
        if (
            ...
            and (not report["escalated"])
        ):
            unique_ips = set([_report["ip"] for _report in report["reports"]])
            if len(unique_ips) >= 3:
                self.files.db.reports.update_one(
                    {"_id": report["_id"]}, {"$set": {"escalated": True}}
                )
                self.files.db.posts.update_one(
                    {"_id": content_id, "isDeleted": False},
                    {
                        "$set": {
                            "isDeleted": True,
                            "mod_deleted": True,
