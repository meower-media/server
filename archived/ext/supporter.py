...
class Supporter:
    ...
    def timestamp(self, ttype, epoch=int(time.time())):
        ...
        if ttype == 1:
            ...
            rtime = self.meower.timestamp(6)-epoch
            if rtime == 0:
                rtime = "now"
            elif rtime < 60:
                rtime = "a moment ago"
                ...
                if rtime == 1:
                    ...
                    rtime = "{0} minutes ago".format(rtime)
            elif rtime < 86400:
                rtime = int(rtime/3600)
                if rtime == 1:
                    rtime = "an hour ago"
                else:
                    rtime = "{0} hours ago".format(rtime)
            elif rtime < 604800:
                rtime = int(rtime/86400)
                if rtime == 1:
                    rtime = "yesterday"
                else:
                    rtime = "{0} days ago".format(rtime)
            elif rtime < 2592000:
                rtime = int(rtime/604800)
                if rtime == 1:
                    rtime = "a week ago"
                else:
                    rtime = "{0} weeks ago".format(rtime)
            elif rtime < 31536000:
                rtime = int(rtime/2592000)
                if rtime == 1:
                    rtime = "a month ago"
                else:
                    rtime = "{0} months ago".format(rtime)
            else:
                rtime = int(rtime/31536000)
                if rtime == 1:
                    rtime = "a year ago"
                else:
                    rtime = "{0} years ago".format(rtime)
            return str(rtime)
