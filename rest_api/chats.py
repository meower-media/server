...
from typing import Optional, TypedDict, List, Literal, Union
from . import APIRoot
...
async def get_chats():
    ...
    user_settings = app.files.db.user_settings.find_one(
        {"_id": request.user}, projection={"active_dms": 1, "favorited_chats": 1}
    )
    if not user_settings:
        user_settings = {"active_dms": [], "favorited_chats": []}
    ...
    # Get chats
    chats = list(
        app.files.db.chats.find(
            {
                "$or": [
                    {  # DMs
                        "_id": {
                            "$in": user_settings["active_dms"]
                            + user_settings["favorited_chats"]
                        },
...
async def create_chat():
    ...
    if (
        app.files.db.chats.count_documents(
            {"type": 0, "members": request.user}, limit=150
        )
        >= 150
    ):
...
async def update_chat(chat_id):
    ...
    # Check restrictions
    if body.nickname and app.security.is_restricted(
        request.user, Restrictions.EDITING_CHAT_NICKNAMES
    ):
        return {"error": True, "type": "accountBanned"}, 403
    ...
    app.supporter.createPost(
        chat_id,
        "Server",
        f"@{request.user} changed the nickname of the group chat to '{chat['nickname']}'.",
...
async def leave_chat(chat_id):
    ...
    if chat["type"] == 0:
        ...
        if len(chat["members"]) > 0:
            ...
            app.files.db.chats.update_one(
                {"_id": chat_id},
                {"$set": {"owner": chat["owner"]}, "$pull": {"members": request.user}},
            )
            ...
            app.supporter.createPost(
                chat_id,
                "Server",
                f"@{request.user} has left the group chat.",
                chat_members=chat["members"],
            )
        else:
        ...
        app.files.db.user_settings.update_one(
            {"_id": request.user}, {"$pull": {"active_dms": chat_id}}
        )
    else:
        abort(500)
    ...
    app.supporter.sendPacket(
        {"cmd": "direct", "val": {"mode": "delete", "id": chat_id}, "id": request.user}
    )
...
async def add_chat_member(chat_id, username):
    ...
    if (
        app.files.db.relationships.count_documents(
            {
                "$or": [
                    {"_id": {"from": request.user, "to": username}, "state": 2},
                    {"_id": {"from": username, "to": request.user}, "state": 2},
                ]
            },
            limit=1,
        )
    ...
    app.supporter.createPost(
        "inbox",
        username,
        f"You have been added to the group chat '{chat['nickname']}' by @{request.user}!",
    )
    ...
    app.supporter.createPost(
        chat_id,
        "Server",
        f"@{request.user} added @{username} to the group chat.",
        chat_members=chat["members"],
    )
...
async def remove_chat_member(chat_id, username):
    ...
    app.supporter.createPost(
        "inbox",
        username,
        f"You have been removed from the group chat '{chat['nickname']}' by @{request.user}!",
    )
    ...
    app.supporter.createPost(
        chat_id,
        "Server",
        f"@{request.user} removed @{username} from the group chat.",
        chat_members=chat["members"],
    )
...
async def transfer_chat_ownership(chat_id, username):
    ...
    app.supporter.createPost(
        chat_id,
        "Server",
        f"@{request.user} transferred ownership of the group chat to @{username}.",
        chat_members=chat["members"],
    )
