...
class supporter:
    ...
    def validate(self, keys: dict, payload: dict, optional=None, sizes: dict = None):
        ...
        self.log(f"Running validator: {keys}, {payload}, {optional}, {sizes}")
        ...
        for key in keys.keys():
            ...
            if (key in payload) or (key in optional):
                ...
                if (key not in payload) and (key in optional):
                    self.log(f"Validator: Payload {payload} key {key} is optional")
                    continue
                ...
                if type(keys[key]) == list:
                    ...
                    if not type(payload[key]) in keys[key]:
                        self.log(f"Validator: Payload {payload} key {key} value is invalid type. Expecting {keys[key]}, got {type(payload[key])}")
                        return self.invalid

                    # Check if the size of the payload is too large 
                    if sizes:
                        if (key in sizes.keys()) and (len(str(payload[key])) > sizes[key]):
                            self.log(f"Validator: Payload {payload} key {key} value is too large")
                ...
                self.log(f"Validator: Payload {payload} is missing key {key}")
                return self.missing_key

        # Hooray, the message is sane
        self.log(f"Validator: Payload {payload} is valid")
        return self.valid
